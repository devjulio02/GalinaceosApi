...
class Galinaceo:
    def __init__(self, id, sist_cria, niv_terr, cod_terr, nom_terr, cl_gal, gal_total):
        self.id = id
...
class GalinaceoSchema(Schema):
    ...
    cod_terr = fields.Str(allow_none=True) # Pode ser nulo/vazio conforme o CSV
    nom_terr = fields.Str(required=True)
    cl_gal = fields.Str(required=True)
    gal_total = fields.Int(required=True)